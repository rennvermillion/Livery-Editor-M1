...
// Step 1: Map the vehicle names (data-value) to the GLB model URLs
const vehicleModels = {
  camaro: 'https://cdn.jsdelivr.net/gh/sooshboi/CARMODELS@main/models/CAMAROUnibody.glb',
  charger: 'https://cdn.jsdelivr.net/gh/sooshboi/CARMODELS@main/models/chargerunibody-compressed.glb',
  m2: 'https://cdn.jsdelivr.net/gh/sooshboi/CARMODELS@main/models/M2Unibody.glb',
  supra: 'https://cdn.jsdelivr.net/gh/sooshboi/CARMODELS@main/models/MK5Unibody-compressed.glb',
};

// Step 2: Get references to buttons and the model viewer
const buttons = document.querySelectorAll('.option');
const viewer = document.querySelector('model-viewer');

// Step 3: Add click listeners to each button
buttons.forEach(button => {
  button.addEventListener('click', () => {
    const vehicleKey = button.getAttribute('data-value');
    const modelUrl = vehicleModels[vehicleKey];
    
    if (modelUrl) {
      viewer.setAttribute('src', modelUrl);
    }

    // Optional: Add active state (highlight selected button)
...
// When a panel button is clicked, remember which panel you selected
document.querySelectorAll('.option[data-type="panel"]').forEach(btn => {
  btn.addEventListener('click', () => {
    selectedPanel = btn.dataset.value;
    console.log('Selected panel:', selectedPanel);
  });
});

// When a color button is clicked, apply color to selected panel
document.querySelectorAll('.option[data-type="color"]').forEach(btn => {
  btn.addEventListener('click', () => {
    if (!selectedPanel) return;

    const color = JSON.parse(btn.dataset.color);
    // Find the material by name
    const material = viewer.model.getMaterialByName(selectedPanel);
...
// When a wrap button is clicked, apply finish to selected panel
document.querySelectorAll('.option[data-type="wrap"]').forEach(btn => {
  btn.addEventListener('click', () => {
    if (!selectedPanel) return;
